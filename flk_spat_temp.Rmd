...
Table 4 shows EOO, AOO and number of locations for selected taxa collected in the Falkland Islands during `r last(year_grps)`.


## Site-level assessment

Figure 3 shows seaweed taxonomic richness at sites surveyed during the DPLUS068 project during 2018--19.
...
#   cat("\n")  # line break following each print call
# }
```
...
```{r taxa_eoo_aoo_locs_table, echo=FALSE}
taxa_eoo_aoo_locs %>%
  # format numerical columns:
  mutate_at(2:3, ~ formatC(., big.mark = ",", digits = 0, format = "f")) %>%
  kable(
    caption = "Extent of occurrence (EOO), area of occupancy (AOO) and number of locations for selected seaweed taxa collected from the Falkland Islands during `r last(year_grps)`.",
