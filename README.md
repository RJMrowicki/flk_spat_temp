...
Data files in `/data` folder (`flk_sites_DPLUS068.csv`, `flk_specimens_DPLUS068.csv` and `flk_specimens_herb.csv`) are derived from master `flk_sites_specimens_DPLUS068.xlsx` workbook in `../field_data` folder and 'combined working' sheet of `flk_specimens.xlsx` in `./flk_specimens` folder.
